...
def update_one_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    ...
    return sandwiches.update(db=db, sandwiches=sandwich, sandwiches_id=sandwich_id)
...
def create_resource(resource: schemas.ResourceCreate, db: Session = Depends(get_db)):
    return resources.create(db=db, resources=resource)
...
def update_one_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    ...
    if resource_db is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return resources.update(db=db, resources=resource, resource_id=resource_id)
...
def delete_one_resource(resource_id: int, db: Session = Depends(get_db)):
    ...
    if resource is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return resources.delete(db, resource_id=resource_id)
...
def read_one_recipes(recipes_id: int, db: Session = Depends(get_db)):
...
def update_one_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    ...
    if recipe_db is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.update(db=db, recipe_id=recipe_id, recipes=recipe)


@app.delete("/recipes/{recipes_id}", tags=["Recipes"])
def delete_one_recipes(recipe_id: int, db: Session = Depends(get_db)):
    recipe = recipes.read_one(db, recipe_id=recipe_id)
    if recipe is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.delete(db, recipe_id=recipe_id)
...
@app.get("/order_details/{order_details_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def read_one_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = order_details.read_one(db, order_detail_id=order_detail_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_detail

@app.put("/order_details/{order_details_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
...
def delete_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    ...
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return order_details.delete(db, order_detail_id=order_detail_id)